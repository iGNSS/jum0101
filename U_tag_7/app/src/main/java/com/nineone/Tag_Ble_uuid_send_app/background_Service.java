...
public class background_Service extends Service implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        String accX = String.format(Locale.KOREA, "%.3f" , arrayfloat[0]);
        String accY = String.format(Locale.KOREA, "%.3f" , arrayfloat[1]);
        String accZ = String.format(Locale.KOREA, "%.3f" , arrayfloat[2]);
        String gyroX = String.format(Locale.KOREA, "%.3f" , arrayfloat[3]);
        String gyroY = String.format(Locale.KOREA, "%.3f" , arrayfloat[4]);
        String gyroZ = String.format(Locale.KOREA, "%.3f" , arrayfloat[5]);

        // Log.e("onAccuracyChange",arrayfloat[0]+","+ arrayfloat[1]+","+  arrayfloat[2]+","+  arrayfloat[3]+","+  arrayfloat[4]+","+  arrayfloat[5]);
       // fasf = Utag_Arrary(Float.parseFloat(accX),Float.parseFloat(accY), Float.parseFloat(accZ),Float.parseFloat(gyroX), Float.parseFloat(gyroY),Float.parseFloat(gyroZ));
        ...
        if (!mService_Advertiserstart) {
            ...
            mService_Advertiserstart = true;
            listcange_handler.postDelayed(runnable10, 250);
            //
        }
